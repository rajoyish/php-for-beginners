					...
					<div class="relative ml-3">
						<div>
							...
							<?php } else { ?>
								...
								<a href="/login" class="<?= urlIs('/register') ? 'bg-gray-900 text-white' : 'text-gray-300' ?> hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Login</a>
							<?php } ?>
						</div>
					</div>
					<?php if ($_SESSION['user'] ?? false) { ?>
						<div class="ml-3">
							<form action="/session" method="POST">
								...
								<button type="submit" class="bg-gray-900 text-white hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Logout</button>
							</form>
						</div>
					<?php } ?>
				</div>
